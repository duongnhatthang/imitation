...
def test_detect_t_sat_upward_non_saturating_is_rejected():
    """Curve that stays flat at the initial level is not 'saturated up'."""
    flat = [0.05] * 60
    obs = list((np.arange(60) + 1) * 50)
    t_sat, sat_val = detect_t_sat(
        flat, obs, smooth_window=10, metric_direction="up"
    )
    # final_mean >= 1.2 * initial_mean is NOT satisfied → reject.
